...
public class Bomberman_Player2 : MonoBehaviour {
	...
	void Update () {
		...
		anim.SetFloat ("Speed", Input.GetAxis ("Vertical_Player2")); 

		// Starten der Animation zum Seitwärtslaufen
		anim.SetFloat ("SideSpeed", Input.GetAxis ("Horizontal_Player2")); 

		// Starten der Animation zum Bombenwurf
		anim.SetFloat ("Throw", Input.GetAxis ("Jump_Player2")); 

		// Translokation der Spielfigur
		CharacterController controller = GetComponent<CharacterController>();
		if (controller.isGrounded) {
			moveDirection = new Vector3(Input.GetAxis("Horizontal_Player2"), 0, Input.GetAxis("Vertical_Player2"));
			moveDirection = transform.TransformDirection(moveDirection);
			moveDirection *= speed;
		}

		// Rotation der Spielfigur
		gameObject.transform.RotateAround (gameObject.transform.position, Vector3.up, Input.GetAxis ("Mouse X_Player2") * 5); 
		...
		if (timeBetweenBombThrows <= 0 && Input.GetAxis ("Jump_Player2") >= 0.8) {
